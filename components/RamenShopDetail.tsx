...
const CongestionIndicator: React.FC<{ status: RamenShop['congestion'] }> = ({ status }) => {
    ...
    return (
        <span className={`inline-flex items-center text-sm font-semibold px-3 py-1 rounded-full ${bgColor} ${color}`}>
            {text}
        </span>
    );
};
    ...
    <div className="animate-slide-in-right bg-white dark:bg-gray-900 pb-20">
      ...
      <DetailSection title="店舗情報" icon={<Clock className="w-5 h-5"/>}>
        <ul className="space-y-3 text-gray-700 dark:text-gray-300">
          <li className="flex items-center"><span className="w-5 h-5 mr-3" /><span>{shop.hours}</span></li>
      ...
      <DetailSection title="アクセス" icon={<Train className="w-5 h-5" />}>
          <p className="text-gray-700 dark:text-gray-300">{shop.accessInfo}</p>
      </DetailSection>
      
      <DetailSection title="混雑状況" icon={<Users className="w-5 h-5" />}>
      ...
      <DetailSection title="駐車場情報" icon={<ParkingSquare className="w-5 h-5" />}>
        <p className="text-gray-700 dark:text-gray-300">{shop.parkingInfo}</p>
      </DetailSection>

      <DetailSection title="クチコミ" icon={<MessageSquare className="w-5 h-5" />}>
        <ReviewAccordion reviews={shop.reviews} />
      </DetailSection>

      <DetailSection title="ストリートビュー" icon={<MapPin className="w-5 h-5" />}>
        <ResponsiveImage photo={streetViewPhoto} sizes="(max-width: 768px) 100vw, 800px" className="w-full h-48 object-cover rounded-lg dark:opacity-80" />
        <p className="text-xs text-center text-gray-500 dark:text-gray-400 mt-2">現在地から徒歩 約{Math.round(shop.distance / 80)}分 / 車 約{Math.round(shop.distance / 400)}分</p>
      </DetailSection>
      
      <DetailSection title="みんなの投稿" icon={<MessageSquare className="w-5 h-5" />}>
         <UserPostForm placeId={shop.placeId} onAddPost={onAddPost} showToast={showToast} />
