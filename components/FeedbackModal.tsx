...
const FeedbackModal: React.FC<FeedbackModalProps> = ({ isOpen, onClose, onSubmit, showToast }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!details.trim()) {
            showToast('詳細を入力してください。'); // Replaced alert
            return;
        }
        ...
        try {
            ...
            // この部分は、VercelのAPIルートに置き換える必要があります。
            // 現状はダミーの成功/失敗ロジックに置き換えます。
            const response = await new Promise<Response>((resolve, reject) => {
                setTimeout(() => {
                    if (Math.random() > 0.1) { // 90% success rate
                        resolve(new Response(JSON.stringify({ message: "Feedback submitted" }), { status: 200 }));
                    } else {
            ...
            showToast('フィードバックの送信に失敗しました。後でもう一度お試しください。'); // Replaced alert
        } finally {
